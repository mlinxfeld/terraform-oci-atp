...
resource "oci_database_autonomous_database" "FoggyKitchen_ATP_remote_database" {
  ...
  source                         = "CROSS_REGION_DATAGUARD" # CROSS_REGION_DISASTER_RECOVERY
  remote_disaster_recovery_type  = var.remote_data_guard_enabled ? "CROSS_REGION_DATAGUARD" : null # CROSS_REGION_DISASTER_RECOVERY
